...
This compiles the client into a jar located at `./build/libs/api-queue-gateway-0.0.1.jar`. Note that this jar does not include any dependencies.
...
```perl
dependencies {
    compile 'com.example.apigateway:api-queue-gateway:0.0.1'
}
```
...
<dependencies>
    <dependency>
        ...
        <artifactId>api-queue-gateway</artifactId>
...
This will compile the client and package all dependencies into a jar located at `./build/libs/api-queue-gateway-0.0.1-all.jar`.
...
```java
ApiQueueGatewayClientBuilder builder = ApiQueueGateway.builder();
```
...
```java
ApiQueueGateway client = ApiQueueGateway.builder()
...
```java
ApiQueueGateway client = ApiQueueGateway.builder().build();
client.shutdown();
...
## Making requests
After a client is configured and created, you can make a request to the service. A method on the client interface (`ApiQueueGateway`) is created for all actions (resource + method) in your API.
...
```java
ApiQueueGateway client = ApiQueueGateway.builder().build();
PostMessagesResult result = client.postMessages(new PostMessagesRequest());
```
...
```java
ApiQueueGateway client = ApiQueueGateway.builder().build();
client.postMessages(new PostMessagesRequest().sdkRequestConfig(
...
Service exceptions and client exceptions can be handled separately. Any exceptions modeled in the API will be a subtype of ApiQueueGatewayException.
```java
try {
    client.postMessages(...);
} catch(ApiQueueGatewayException e) {
   // All service exceptions will extend from ApiQueueGatewayException.
   // Any unknown or unmodeled service exceptions will be represented as a ApiQueueGatewayException.
